...
export class Track implements TrackData {
  ...
  public static async from(
    ...
    logger.info(`Validating input: "${cleanInput}" -> detected type: ${type}`);

    // 1. Handle Spotify
    if (type && type.includes('sp_')) {
      if (play.is_expired()) {
    ...
    if (type && type.includes('so_')) {
      if (type === 'so_track') {
        ...
        const info = await play.soundcloud(cleanInput) as any; // SoundCloudPlaylist
        const tracks = await info.all_tracks();
    ...
    // 4. Search fallback (default to YouTube search)
    ...
    throw new Error('Không tìm thấy bài hát nào từ nguồn yêu cầu.');
  }
}
