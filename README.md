...
Khi deploy bot nhạc lên các máy chủ cloud (như Google Cloud, AWS, Render), YouTube thường chặn toàn bộ IP của dải máy chủ này và yêu cầu xác thực bot bằng **PO Token (Proof of Origin Token)**. 

**Tin vui:** Hiện tại dự án đã được tích hợp sẵn một bộ **PO Token** và **Visitor Data** mặc định. Điều này giúp bot có thể hoạt động ngay lập tức trên hầu hết các môi trường cloud mà không cần cấu hình thêm.

Tuy nhiên, nếu bộ token mặc định này hết hạn hoặc bạn muốn sử dụng token riêng của mình, bạn vẫn có thể cấu hình thông qua hai biến môi trường `YT_PO_TOKEN` và `YT_VISITOR_DATA` trong file `.env`:

### Cách lấy PO Token và Visitor Data mới (nếu cần):
